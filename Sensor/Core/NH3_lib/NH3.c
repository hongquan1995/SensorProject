...
uint8_t getNH3HumidityValue(float *data)
{
	...
	indexBuffer = 0;
	memset(buffer, '\0', 256);
	result = Master_Read_Modbus(ADDRESS_SLAVE_NH3, FUNCODE_COMMON_NH3, REG_ADDRESS_HUMI, LENGTH_DATA_HUMI);
	if(result != HAL_OK){
		return FALSE;
		//my_printf("not send frame");
	}
...
uint8_t getNH3(float *data){
	...
	indexBuffer = 0;
	memset(buffer, '\0', 256);
	result = Master_Read_Modbus(ADDRESS_SLAVE_NH3, FUNCODE_COMMON_NH3, REG_ADDRESS_HUMI, LENGTH_DATA_NH3);
	if(result != HAL_OK){
		return FALSE;
		//my_printf("not send frame");
	}
